...
class _ConsoleDetailPageState extends State<ConsoleDetailPage> {
  ...
  ///订单详情
  OrderMasterEntity orderMasterEntity=new OrderMasterEntity(orderRounds: List());
  ...
  _getOrderDetail() async {
    ...
    await HttpGo.getInstance()
        .get(UrlPath.orderInfoPath + widget.orderId.toString() +
        "/info")
        .then((baseResult) {
      print("订单详情成功");
      setState(() {
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              return InkWell(
                                onTap: (){
                                  setState(() {
                                    type="2";
                                    roundId=orderMasterEntity.orderRounds[index].id;
                                  });
                                },
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: orderMasterEntity.orderRounds[index].id == roundId
                                        ? Colors.lightBlue
                                        : Colors.white,
                                    border: Border.all(color: Colors.grey,width: 2)
                                  ),
                                  height: ScreenUtil.getInstance().setWidth(80),
                                  child:Text(CommonUtils.getLocale(context).round + " " +orderMasterEntity.orderRounds[index].num.toString()),
                                ),
                              );
                            }),
                      ),
                    ...
                    Expanded(
                      ...
                      child:Column(
                        children: <Widget>[
                          ...
                          ///订单详情
                          Expanded(
                            child: Record(type,widget.orderId,round: roundId,),
                          ),
                        ],
                      ),
                    ),
